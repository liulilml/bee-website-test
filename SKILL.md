...
5. **必须关闭标签页** → 回到列表继续下一个
6. 格式：`[X/N] DApp名 → PASS/FAIL/WARN - 详情`
...
#### 9a. APK 下载
`exec curl -sI <APK链接>` 检查 APK 链接可达性（不实际下载完整文件）
...
**封禁特征（→ FAIL）：**
- Twitter: "Account suspended"、"doesn't exist"、"This account doesn't exist"
- Facebook: "content isn't available"、"page has been removed"、"This page isn't available"
- TikTok: "Couldn't find this account"、"account was banned"
- YouTube: "channel is not available"、"account has been terminated"
- Medium: 404 页面、"doesn't have a page"
- LinkedIn: "page not found"、页面不存在
- Telegram: 页面完全无内容、"If you have Telegram, you can view" 都看不到
- Discord: "Invite Invalid"、"invite may be expired"

**格式：**
```
[1/8] Twitter/X → PASS - @beenetworkintl, XX followers, XX posts
[2/8] TikTok → PASS - @beenetwork_official, XX followers
...
[8/8] Discord → PASS - Invite valid, server "Bee Network"
```
...
- **超时**：整体不超过 90 分钟（从 60 分钟调整为 90 分钟以确保全量覆盖）
