...
⚠️ **强制要求：必须对当前页面加载出的所有游戏执行点击验证，不能抽样，不能跳过。**
...
3. **每验证 20 个游戏，`browser screenshot` 截图保存**（文件名：`games_screenshot_XXX.png`）

**进度汇报要求：**
- 每验证 20 个游戏，打印一次进度（如 "Games 验证进度：20/100"）
- 完成后汇报总数（如 "Games 全量验证完成：100/100"）
...
⚠️ **强制要求：必须对每一个 DApp 卡片执行点击验证，不能抽样，不能跳过。**
...
**进度汇报要求：**
- 每验证 50 个 DApp，打印一次进度（如 "DApp 验证进度：50/354"）
- 完成后汇报总数（如 "DApp Store 全量验证完成：354/354"）
...
1. **搜索存在的 DApp**（至少测试 3 个）：
   ...
   - 验证搜索结果是否正确显示该 DApp
   - `browser screenshot` 截图保存（文件名：`search_exist_XXX.png`）

2. **搜索不存在的 DApp**（至少测试 2 个）：
   - 在搜索框输入随机不存在的名称（如 "NotExistTest12345"、"FakeAppXYZ"）
   - 验证页面是否正常显示"无结果"或空状态
   - **不能出现**：500 错误、404 错误、页面崩溃、空白页
   - `browser screenshot` 截图保存（文件名：`search_notfound_XXX.png`）

3. **记录结果**：
   ```
   搜索测试 | 搜索词 | 预期结果 | 实际结果 | 状态(PASS/FAIL) | 截图路径
   ```
...
2. **`browser screenshot` 截图保存**（文件名：`beehive_news_initial.png`）
3. 找到 **Next slide / Previous slide 按钮**
4. 点击 **Next slide** 按钮：
   - `browser act kind=click ref=<next_button_ref>`
   - `browser snapshot` 获取切换后的内容
   - **验证数据是否发生变化**（新闻标题/日期与切换前不同）
   - **`browser screenshot` 截图保存**（文件名：`beehive_news_next_XXX.png`）
   - 记录切换后显示的新闻列表
5. 再点击 **Next slide** 2-3 次，每次都验证并截图
6. 点击 **Previous slide** 按钮：
   - 验证是否能回退到之前的新闻
   - 验证内容与之前记录的一致
   - **`browser screenshot` 截图保存**（文件名：`beehive_news_prev_XXX.png`）
7. 记录结果：切换按钮是否存在、点击后数据是否正确变化、是否能前后切换
...
2. 与前一天对比
3. 基础页面 + 分类站点 + 新闻/文章
4. **Games 全量测试**（TAB 切换、游戏点击验证、详情页交互、**截图附件**）
5. **DApp Store 全量测试**（分类列表、全部 DApp 点击结果、**搜索功能测试**、深度交互、**截图附件**）
6. AD 表单测试
7. **Bee Hive 测试**（**轮播切换截图**、新闻链接、DApp 入口）
8. 下载功能 + 外部链接
9. 社交媒体链接验证
10. 登录 + 悬浮按钮
11. 已知限制与建议
...
**截图附件说明：**
- Games 截图：每 20 个游戏一张，证明验证过程
- DApp Store 搜索截图：存在/不存在的搜索结果对比
- Bee Hive 轮播截图：初始状态、Next 切换、Previous 切换
- 所有截图上传到报告文件夹，报告中插入图片链接
