...
🚨 **测试结束后必须执行，防止 Chrome 标签页堆积占用内存！**

1. **检查所有打开的标签页**
   ```
   browser tabs → 获取所有标签页列表
   ```
...
2. **关闭所有非主标签页**
   ...
   for each tab in tabs:
     if tab != main_tab:
       browser act kind=close targetId=<tab.id>
   ```

3. **验证清理结果**
   ```
   browser tabs → 确认只剩 1 个主标签页
   ```
...
5. **记录清理结果**
   ```
   标签页清理：关闭 X 个标签页，剩余 1 个
   文件清理：删除 Y 个下载文件
   ```

**⚠️ 注意事项：**
- 每次验证完 DApp/游戏/新闻链接后**必须立即关闭标签页**
- 不要等到最后才一起关闭（容易遗漏）
- 如果中途报错中断，也要执行清理步骤
